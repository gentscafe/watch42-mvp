...
# 2. UI STYLE - ATLAS INSPIRED (Full Integration)
st.markdown("""
    <style>
...
if 'initialized' not in st.session_state:
    ...
    def create_watch(brand, model, is_target=False, year=None):
        ...
        # Trend: Reserve standard moves from ~42h to ~72h
...
with st.sidebar:
    ...
    st.write("---")
    st.caption("Intelligence SaaS v5.2")
...
# --- VIEW 2: PRICING INTELLIGENCE ---
elif view == "Pricing Intelligence":
    col_t1, col_t2 = st.columns([2, 1])
    with col_t1: st.markdown("### Pricing Intelligence")
    with col_t2: 
        target = st.selectbox("Target Watch", st.session_state.my_portfolio, format_func=lambda x: f"{x['Model']} ({x['Year']})")
    ...
    metrics_data = [
        ("Category", target['Category'], None),
        ("Competitors", len(df_f), None),
        ("Market Average", f"€ {avg_p:,.0f}", None),
        ("Price Gap", f"€ {target['Price']:,}", f"{diff:+.1f}%")
    ]
    for m, (label, val, delta) in zip([m1, m2, m3, m4], metrics_data):
        with m:
...
# --- VIEW 3: DESIGN GRID ---
    ...
    with col_m2:
        st.markdown(f'<div class="atlass-card" style="height:495px;"><div class="card-label">Strategic Insights</div><div class="card-value">{len(df_f_design)} Models</div><hr style="border:0; border-top:1px solid #E2E8F0; margin:20px 0;"><div style="font-size:0.9rem; color:#475569;"><b>White Space Detected:</b><br>Low competition in <b>39mm/10mm</b> configurations for the {sel_cat} segment.</div></div>', unsafe_allow_html=True)

# --- VIEW 4: MARKET TRENDS ---
elif view == "Market Trends":
    st.markdown("### Market Intelligence & Trends")
    # Filter for Trend Category
        ...
        st.markdown(f"""
            <div class="atlass-card" style="height:488px;">
                ...
                <div class="insight-box">
                    ...
                    2026 Industrial Standard: <b>{mkt_avg_2026:.0f}h</b>.<br>
                ...
                <div style="margin-top:25px; padding-top:20px; border-top:1px solid #E2E8F0;">
                    ...
                    <div style="color:#10B981; font-size:0.8rem;">Accelerating trend in {trend_cat} segment</div>
                </div>
            </div>
        """, unsafe_allow_html=True)
