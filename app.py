...
# 2. UI STYLE - TOTAL ATLAS DESIGN (v5.8)
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    st.write("---")
    st.caption("Fears Analytics v5.8")
...
if view == "Dashboard":
    ...
    for m, (label, val, delta) in zip([m1, m2, m3, m4], [("Category", target['Category'], None), ("Competitors", len(df_f), None), ("Market Avg", f"€ {avg_p:,.0f}", None), ("Posizione", f"€ {target['Price']:,}", f"{diff:+.1f}%")]):
        with m:
    ...
    fig.add_trace(go.Scatter(x=[target['Price']], y=[target['Reserve']], mode='markers+text', text=[target['Model']], textposition="top center", marker=dict(color='#d4af37', size=15, line=dict(width=2, color='white'))))
    ...
    with col_m1:
        ...
        st.markdown('</div>', unsafe_allow_html=True)
    with col_m2:
        st.markdown(f'<div class="atlass-card" style="height:495px;"><div class="card-label">Strategic Insights</div><div class="card-value">{len(df_f_design)} Models</div><hr style="border:0; border-top:1px solid #E2E8F0; margin:20px 0;"><div style="font-size:0.9rem; color:#475569;"><b>White Space:</b><br>Bassa competizione rilevata nel segmento ultra-thin ({sel_cat}) dove Fears eccelle con i modelli Archival.</div></div>', unsafe_allow_html=True)
    ...
    with c_left:
        st.markdown('<div class="atlass-card">', unsafe_allow_html=True)
        st.markdown('<div class="card-label">Industrial Standard Shift (Reserve)</div>', unsafe_allow_html=True)
    ...
    with c_right:
        st.markdown(f"""
            <div class="atlass-card" style="height:488px;">
                <div class="card-label">AI Strategy Control</div>
                <div class="insight-box">
                    <b>Obsolescence Risk</b><br>
                    {max_yr} Industrial Standard: <b>{mkt_avg_2026:.0f}h</b>.<br>
                    <i>Status: Review manual calibers legacy.</i>
                </div>
